...
1. **Screen a role** — paste a job posting and get a 1–10 match score, a verdict (apply / review / pass), a salary recommendation, fit reasons, gaps, and a plain-language summary. If you've screened a role at the same company before, it surfaces that history — prior title, score, and verdict — right above the new result, so a repeat posting at a company you've already evaluated doesn't get scored in a vacuum.
...
* A lightweight screening history (company, title, score, verdict, date — no job description text or resume content) is saved separately in local storage so repeat companies can be flagged. It's used only for the same-company banner described above; it isn't sent back through the model or used to change how anything is scored.
* The only network call this page makes is directly from your browser to Anthropic's API, using your own key.
* Use Start over (in the Resume & rules tab) to clear your saved resume and rules, and redo setup — useful if you want to screen for someone else, or start fresh. This does not clear screening history; clear that separately from your browser's local storage for this page if you want a clean slate there too.
